...
def age(tab):
    with tab:
        ...
        with col1:
            fig = px.bar(age_data_pc, x='Age Range', y='% Rejoin', text_auto=True, barmode='group', title='Rejoin by age')
            st.plotly_chart(fig)
        with col2:
            fig = px.bar(age_data_pc, x='Age Range', y='% Stay out', text_auto=True, color_discrete_sequence=["red"], title="Stay out by age")
            st.plotly_chart(fig)
