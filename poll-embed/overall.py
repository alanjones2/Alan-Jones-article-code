...
def overall(tab):
    with tab:
            ...
            with col1:
                ...
                """)
            with col2:
                fig = px.bar(result, x='Original Vote', y=['Rejoin%','Stay%','DK%','NoVote%'], text_auto=True, barmode='group',width=420,height=420)
                st.plotly_chart(fig)
            ...
            with col3:
                ...
                """)
            with col4:
                fig = px.bar(result, x='Original Vote', y=['Rejoin%','Stay%','DK%','NoVote%'], text_auto=True, barmode='stack',width=420,height=420)
                st.plotly_chart(fig)
            ...
            with col5:
                ...
                """)
            with col6:
                fig = px.bar(result[result['Original Vote']=='All'], x='Original Vote', y=['Rejoin%','Stay%','DK%','NoVote%'], text_auto=True, barmode='group',width=420,height=420)
                st.plotly_chart(fig)
            ...
            with col8:
                fig = px.bar(y=[55.9,44.1],  
                            x=['Rejoin%','Stay%'], 
                            text_auto=True,width=420,height=420)
                st.plotly_chart(fig)
